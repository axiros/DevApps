...
import sys, os
from .func_sigs import signature, pretty_type
import textwrap
...
def render_help(app, level=None, h=None):
    ...
    level = len(level)
    h = h or 1
    md = _show_help(app, termsize(), level, sh_code, as_md, h)
    return md
...
def _show_help(app, size, level, sh_code, as_md, h):
    md, inner, ad = [], [], []
    ...
    def params(L, size=size, as_md=as_md):
        cols = size[-1]
        l = L
        for line in L:
    ...
    for attr in app.__attrs_attrs__:
        ...
        if hasattr(v, 'factory'):
            inn = getattr(app, n)
            ap(inner, _show_help(inn, size, level - 1, sh_code, as_md, h=2))
        else:
            t = attr.type
            if n == 'xb_dflt_True':
                breakpoint()
    ...
    def func(n, func, level=level, size=size, h=h, md=md, sh_code=sh_code):
        ...
        if not sh_code:
            ap(md, '')
            ap(md, '> %s%s' % (func.__name__, pretty_type(signature(func))))
